  ...
  <view class="uni-container">
    <uni-forms ref="form" :value="formData" validateTrigger="bind">
	  ...
	  <uni-forms-item  label="密码">
	    <uni-easyinput placeholder="密码" v-model="password" trim="both"></uni-easyinput>
	  </uni-forms-item>
  ...
  export default {
    ...
    methods: {
      ...
      async submitForm(value) {
		...
		value.department_id=value.department_id[0];
		console.log(value);
		let result=await this.$util.cloudFunction("user","add",value);
		if(result.code==0){
			uni.showToast({
			  title: '新增成功'
			});
