<template>
  <view class="uni-container">
    <uni-forms ref="form" :value="formData" validateTrigger="bind">
     ...
     <uni-forms-item name="url" label="图片列表">
     	<uni-file-picker file-mediatype="image" :limit="1" return-type="object" v-model="formData.url" />
     </uni-forms-item>
      ...
      <uni-forms-item name="enable" label="是否开启">
...
<script>
  import { validator } from '../../js_sdk/validator/opendb-app-menus.js';

  const db = uniCloud.database();
  const dbCmd = db.command;
  const dbCollectionName = 'opendb-app-menus';
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("menu_id,name,icon,url,sort,parent_id,permission,enable").get().then((res) => {
